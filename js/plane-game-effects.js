/* Game Bắn máy bay — hiệu ứng canvas: nền vũ trụ (tinh vân + 3 lớp sao trôi), hạt nổ có vật lý
   ...
   Hạt dùng bộ máy chung js/game-particles.js (nạp trước file này). */
...
function createSpaceFx() {
  ...
  return { w: 0, h: 0, bg: null, stars: [], ps: createParticles(FX_MAX_PARTS, reduced), rings: [], texts: [], shake: 0, flash: 0, flashColor: '#fff', reduced };
}
...
function spaceFxEvent(fx, e) {
  if (e.type === 'fire') {
    burst(fx.ps, e.x + Math.cos(e.a) * 14, e.y + Math.sin(e.a) * 14, 3, { kind: 'spark', a: e.a, spread: 0.6, speed: 180, life: 0.12, size: 2, colors: ['#bff6ff'] });
  } else if (e.type === 'hit') {
    burst(fx.ps, e.x, e.y, 8, { kind: 'spark', a: e.a + Math.PI, spread: 2.2, speed: 260, life: 0.3, size: 2, colors: ['#ffe28a', '#ff9f43'] });
    ...
    burst(fx.ps, e.x, e.y, 34 * big, { kind: 'spark', speed: 340 * big, life: 0.55, size: 2.2, colors: bad ? ['#ff6b6b', '#ffd3d3'] : ['#fff4c2', '#ffc24b', '#ff7a3c'] });
    burst(fx.ps, e.x, e.y, 10 * big, { kind: 'debris', speed: 170 * big, life: 1.1, size: e.r * 0.35, colors: [col], drag: 0.6, g: 160 });
    burst(fx.ps, e.x, e.y, 7 * big, { kind: 'smoke', speed: 50, life: 0.9, size: e.r * 0.9, colors: ['#8b93b8'], drag: 1.2 });
